...
if (Test-Path "./RE3InventoryOverlay.zip") {
    Remove-Item "./RE3InventoryOverlay.zip"
}

$modinfo=@"
name=RE3 Inventory Overlay
version=v$version
description=Inventory overlay for Resident Evil 3 (RT and non-RT). Requires REFRamework and reframework-d2d
...
& luacheck .\RE3InventoryOverlayNonBinarySearch.lua --config .\.luacheckrc
if ($LastExitCode -ne 0) {
    Write-Error "luacheck failed"
    exit
}

Copy-Item "./RE3InventoryOverlayNonBinarySearch.lua" -Destination "./output/reframework/autorun"
...
Compress-Archive -Path "./output/*" -DestinationPath "RE3InventoryOverlay.zip"